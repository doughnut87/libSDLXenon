...
int  SDL_SYS_CDInit(void)
{
	...
	if (LOUSHORT(mciSendCommand(0,MCI_SYSINFO, MCI_SYSINFO_QUANTITY | MCI_WAIT, (PVOID)&msp, 0)) != MCIERR_SUCCESS) {
		return(CD_ERROR);
	}
...
static int SDL_SYS_CDOpen(int drive)
{
	...
	/* Open the device */
	mop.hwndCallback = NULLHANDLE;	/* None */
	mop.usDeviceID = 0;		/* Will be returned. */
	mop.pszDeviceType = (PSZ)SDL_cdlist[drive];		/* CDAudio Device */
	if (LOUSHORT(mciSendCommand(0,MCI_OPEN,MCI_WAIT,&mop, 0)) != MCIERR_SUCCESS) {
		return(CD_ERROR);
	}
	/* Set time format */
	msp.hwndCallback = NULLHANDLE;		/* None */
	msp.ulTimeFormat = MCI_FORMAT_MSF;	/* Minute : Second : Frame structure */
	msp.ulSpeedFormat = 0;		/* No change */
	msp.ulAudio = 0;		/* No Channel */
	msp.ulLevel = 0;		/* No Volume */
	msp.ulOver = 0;			/* No Delay */
	msp.ulItem = 0;			/* No item */
	msp.ulValue = 0;		/* No value for item flag */
	if (LOUSHORT(mciSendCommand(mop.usDeviceID,MCI_SET,MCI_WAIT | MCI_SET_TIME_FORMAT,&msp, 0)) == MCIERR_SUCCESS) {
		return (mop.usDeviceID);
	}
	/* Error setting time format? - Close opened device */
	mgp.hwndCallback = NULLHANDLE;		/* None */
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom)
{
	...
	msp.hwndCallback = NULLHANDLE; /* None */
	msp.ulReturn = 0; /* We want this information */
	msp.ulItem = MCI_STATUS_NUMBER_OF_TRACKS;
	msp.ulValue = 0; /* No additional information */
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position)
{
	...
	if (position != NULL) { /* The SDL $&$&%# CDROM call sends NULL pointer here! */
		if ((status == CD_PLAYING) || (status == CD_PAUSED)) {
			...
			msp.ulValue = 0; /* No additiona info */
			if (LOUSHORT(mciSendCommand(cdrom->id,MCI_STATUS,MCI_WAIT | MCI_STATUS_ITEM,&msp, 0)) != MCIERR_SUCCESS) {
				return (CD_ERROR);
			}
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length)
{
	...
	/* Now play it. */
	mpp.hwndCallback = NULLHANDLE;		/* We do not want the info. temp */
	if (LOUSHORT(mciSendCommand(cdrom->id,MCI_PLAY,MCI_FROM | MCI_TO,&mpp, 0)) == MCIERR_SUCCESS) {
		return 0;
	}
	return (CD_ERROR);
}
...
static int SDL_SYS_CDPause(SDL_CD *cdrom)
{
	...
	mgp.hwndCallback = NULLHANDLE;		/* None */
	if (LOUSHORT(mciSendCommand(cdrom->id,MCI_PAUSE,MCI_WAIT,&mgp, 0)) == MCIERR_SUCCESS) {
		return 0;
	}
	return(CD_ERROR);
}
...
static int SDL_SYS_CDResume(SDL_CD *cdrom)
{
	...
	mgp.hwndCallback = NULLHANDLE;		/* None */
	if (LOUSHORT(mciSendCommand(cdrom->id,MCI_RESUME,MCI_WAIT,&mgp, 0)) == MCIERR_SUCCESS) {
		return 0;
	}
	return(CD_ERROR);
}
...
static int SDL_SYS_CDStop(SDL_CD *cdrom)
{
	...
	/* Now stops the media */
	mgp.hwndCallback = NULLHANDLE;		/* None */
	if (LOUSHORT(mciSendCommand(cdrom->id,MCI_STOP,MCI_WAIT,&mgp, 0)) == MCIERR_SUCCESS) {
		return 0;
	}
	return(CD_ERROR);
}
...
static int SDL_SYS_CDEject(SDL_CD *cdrom)
{
	...
	msp.ulItem = 0;			/* No item */
	msp.ulValue = 0;		/* No value for item flag */
	if (LOUSHORT(mciSendCommand(cdrom->id,MCI_SET,MCI_WAIT | MCI_SET_DOOR_OPEN,&msp, 0)) == MCIERR_SUCCESS) {
		return 0;
	}
	return(CD_ERROR);
}
